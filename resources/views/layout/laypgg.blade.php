...
<head>
    ...
    <!-- <link rel="stylesheet" type="text/css" href="assets/back/app-assets/vendors/css/charts/apexcharts.css"> -->
    <link rel="stylesheet" type="text/css" href="assets/back/app-assets/vendors/css/extensions/tether-theme-arrows.css">
    <link rel="stylesheet" type="text/css" href="assets/back/app-assets/vendors/css/extensions/tether.min.css">


    <link rel="stylesheet" type="text/css" href="assets/back/app-assets/vendors/css/tables/datatable/datatables.min.css">
    <!-- END: Vendor CSS-->
...
<body class="vertical-layout vertical-menu-modern semi-dark-layout 2-columns  navbar-floating footer-static" data-menu="vertical-menu-modern">

    <!-- BEGIN: Header-->
    <nav class="header-navbar navbar-expand-lg navbar navbar-with-menu floating-nav navbar-light navbar-shadow navbar-primary">
                    ...
                    <ul class="nav navbar-nav float-right">
                        ...
                        <li class="dropdown dropdown-notification nav-item">
                            ...
                            <?php 
                                ...
                                $notif = DB::SELECT("select*from dumas a, pengguna b, tindak_lanjut c where NOT EXISTS (SELECT * FROM respon WHERE a.DUMAS_ID = respon.DUMAS_ID) and a.PENG_ID = b.PENG_ID and a.DUMAS_ID = c.DUMAS_ID and c.STATUS = 'selesai' and a.PENG_ID = '$ses' and a.HAPUS = 0");
                            ?>
                            @if($notif == null)
                            @else
                            <span class="badge badge-pill badge-danger badge-up" style="padding:7px;">    </span>
                            @endif
                            </a>
                            <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                                ...
                                <li class="scrollable-container media-list">
                                    <a class="d-flex justify-content-between" href="{{ url('/pdataresdumas')}}">
                                        <div class="media d-flex align-items-start">
                                            ...
                                            @else
                                                ...
                                                <div class="media-body">
                                                    <h6 class="danger media-heading yellow darken-3">Pengaduan membutuhkan respon</h6>
                                                    <small class="notification-text">Beberapa pengaduan membutuhkan respon anda</small>
                                                </div>
                                            @endif
                                        </div>
                                    </a>
                        ...
                        <?php 
                            $ses = Session::get('akun');
                            $upses = DB::SELECT("select*from pengguna where PENG_ID = '$ses'");
                            foreach ($upses as $key) {
                        ?> 
                        <li class="dropdown dropdown-user nav-item"><a class="dropdown-toggle nav-link dropdown-user-link" href="#" data-toggle="dropdown">
                                <div class="user-nav d-sm-flex d-none"><span class="user-name text-bold-600">{{$key->NAMA}}</span><span class="user-status">{{$key->LEVEL}}</span></div><span><img class="round" src="assets/foto/{{$key->FOTO}}" alt="avatar" height="40" width="40"></span>
                            </a>
    ...
    <?php $data = DB::SELECT("select*from pengguna");?>
    @foreach($data as $ed)
